...
func consider_moving():
	#we should have each unit consider and display what their possible moves are when you click on them, (or when you hit the preview button)
	#TODO: write a BFS or dijkstra-ish function to analyze the surrounding area and determine which spaces the unit can move to this turn
